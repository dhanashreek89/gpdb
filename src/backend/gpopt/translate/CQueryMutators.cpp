...
CQueryMutators::NeedsProjListNormalization(const Query *query)
{
	...
	ForEach(lc, query->targetList)
	{
		...
		if (!IsA(target_entry->expr, PercentileExpr) && !IsA(target_entry->expr, Aggref) &&
			!IsA(target_entry->expr, GroupingFunc) &&
			!CTranslatorUtils::IsGroupingColumn(
				(Node *) target_entry->expr, query->groupClause, query->targetList))
		{
			return true;
		}
	}
...
CQueryMutators::ShouldFallback(Node *node, SContextTLWalker *context)
{
	...
	if (IsA(node, Const) || IsA(node, Aggref) || IsA(node, PercentileExpr) ||
		IsA(node, GroupingFunc) || IsA(node, SubLink))
	{
		return false;
	}

	TargetEntry *entry = gpdb::FindFirstMatchingMemberInTargetList(node, context->m_target_entries);
	if (NULL != entry && CTranslatorUtils::IsGroupingColumn((Node *) entry->expr,
															context->m_group_clause,
	...
	return gpdb::WalkExpressionTree(
		node, (FallbackWalkerFn) CQueryMutators::ShouldFallback, context);
}
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeGroupByProjList(IMemoryPool *mp,
										 ...
										 const Query *query)
{
	...
	Query *new_query = ConvertToDerivedTable(
		query_copy, false /*should_fix_target_list*/, true /*should_fix_having_qual*/);
	gpdb::GPDBFree(query_copy);

	GPOS_ASSERT(1 == gpdb::ListLength(new_query->rtable));
	Query *derived_table_query =
		(Query *) ((RangeTblEntry *) gpdb::ListNth(new_query->rtable, 0))->subquery;
	...
	ForEach(lc, target_list_copy)
	{
		...
		{
			target_entry->expr =
				(Expr *) FixGroupingCols((Node *) target_entry->expr, target_entry, &context);
		}
	}
	...
	ForEach(lc, target_list_copy)
	{
		...
		{
			target_entry->expr =
				(Expr *) RunGroupByProjListMutator((Node *) target_entry->expr, &context);
			GPOS_ASSERT(
				(!IsA(target_entry->expr, Aggref) && !IsA(target_entry->expr, PercentileExpr)) &&
				!IsA(target_entry->expr, GroupingFunc) &&
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::RunIncrLevelsUpMutator(Node *node, SContextIncLevelsupMutator *context)
{
	if (NULL == node)
	{
		return NULL;
	...
	{
		Query *query =
			gpdb::MutateQueryTree((Query *) node,
	...
	return gpdb::MutateExpressionTree(
		node, (MutatorWalkerFn) CQueryMutators::RunIncrLevelsUpMutator, context);
}
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::RunFixCTELevelsUpMutator(Node *node, SContextIncLevelsupMutator *context)
{
	if (NULL == node)
	{
		return NULL;
	...
	return gpdb::MutateExpressionTree(
		node, (MutatorWalkerFn) CQueryMutators::RunFixCTELevelsUpMutator, context);
}
...
//---------------------------------------------------------------------------
BOOL
CQueryMutators::NeedsLevelsUpCorrection(SContextIncLevelsupMutator *context, Index cte_levels_up)
{
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::RunGroupByProjListMutator(Node *node, SContextGrpbyPlMutator *context)
{
	if (NULL == node)
	{
		return NULL;
	...
	if (IsA(node, Aggref))
	{
		...
		TargetEntry *target_entry = PteAggregateOrPercentileExpr(
			context->m_mp, context->m_md_accessor, (Node *) aggref, attno);

		// Add a new target entry to the query
		context->m_groupby_target_list =
			gpdb::LAppend(context->m_groupby_target_list, target_entry);
	...
	if (IsA(node, PercentileExpr) || IsA(node, GroupingFunc))
	{
		...
		TargetEntry *target_entry =
			PteAggregateOrPercentileExpr(context->m_mp, context->m_md_accessor, pnodeCopy, attno);
	...
	if (!context->m_is_mutating_agg_arg)
	{
		...
		TargetEntry *found_target_entry =
			gpdb::FindFirstMatchingMemberInTargetList(node, context->m_groupby_target_list);
		...
		{
			return (Node *) gpdb::MakeVar(1,  // varno
	...
	return gpdb::MutateExpressionTree(
		node, (MutatorWalkerFn) CQueryMutators::RunGroupByProjListMutator, context);
}
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::RunGroupingColMutator(Node *node, SContextGrpbyPlMutator *context)
{
	if (NULL == node)
	{
		return NULL;
	...
	if (IsA(node, Aggref))
	{
		...
		ForEach(lc, old_aggref->args)
		{
			...
			GPOS_ASSERT(NULL != arg);
			// traverse each argument and fix levels up when needed
			new_args = gpdb::LAppend(new_args,
									 gpdb::MutateQueryOrExpressionTree(
										 arg,
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::FixGroupingCols(Node *node,
								...
								SContextGrpbyPlMutator *context)
{
	...
	TargetEntry *new_target_entry =
		gpdb::MakeTargetEntry((Expr *) expr, (AttrNumber) arity, name, false /*resjunk */);

	new_target_entry->ressortgroupref = orginal_target_entry->ressortgroupref;
	new_target_entry->resjunk = false;

	context->m_groupby_target_list =
		gpdb::LAppend(context->m_groupby_target_list, new_target_entry);
...
//---------------------------------------------------------------------------
TargetEntry *
CQueryMutators::PteAggregateOrPercentileExpr(IMemoryPool *mp,
											 CMDAccessor *md_accessor,
											 Node *node,
											 ULONG attno)
{
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::RunHavingQualMutator(Node *node, SContextHavingQualMutator *context)
{
	if (NULL == node)
	{
		return NULL;
	...
	if (IsA(node, Var))
	{
		...
		if (var->varlevelsup == context->m_current_query_level)
		{
			...
			var->varlevelsup = 0;
			TargetEntry *found_target_entry = gpdb::FindFirstMatchingMemberInTargetList(
				(Node *) var, context->m_groupby_target_list);
	...
	return gpdb::MutateExpressionTree(
		node, (MutatorWalkerFn) CQueryMutators::RunHavingQualMutator, context);
}
...
//---------------------------------------------------------------------------
Var *
CQueryMutators::MakeVarInDerivedTable(Node *node, SContextHavingQualMutator *context)
{
	GPOS_ASSERT(NULL != node);
	...
	const ULONG attno = gpdb::ListLength(context->m_groupby_target_list) + 1;
	TargetEntry *target_entry =
		PteAggregateOrPercentileExpr(context->m_mp, context->m_md_accessor, (Node *) node, attno);
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::FindNodeInTargetEntries(Node *node, SContextHavingQualMutator *context)
{
	GPOS_ASSERT(NULL != node);
	GPOS_ASSERT(NULL != context);
...
//---------------------------------------------------------------------------
Var *
CQueryMutators::IncrLevelsUpInVar(Var *var)
{
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeHaving(IMemoryPool *mp, CMDAccessor *md_accessor, const Query *query)
{
	Query *query_copy = (Query *) gpdb::CopyObject(const_cast<Query *>(query));
	...
	Query *new_query = ConvertToDerivedTable(
		query_copy, true /*should_fix_target_list*/, false /*should_fix_having_qual*/);
	gpdb::GPDBFree(query_copy);
	...
	ForEach(lc, derived_table_query->targetList)
	{
		...
		if (CTranslatorUtils::IsGroupingColumn(target_entry, derived_table_query->groupClause) ||
			CTranslatorUtils::IsSortingColumn(target_entry, derived_table_query->sortClause) ||
	...
	SContextHavingQualMutator context(
		mp, md_accessor, num_target_entries, derived_table_query->targetList);
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeQuery(IMemoryPool *mp,
							   CMDAccessor *md_accessor,
							   const Query *query,
							   ULONG query_level)
{
	...
	Query *pqueryFixedWindowFrameEdge =
		CQueryMutators::PqueryFixWindowFrameEdgeBoundary(pqueryEliminateDistinct);
	gpdb::GPDBFree(pqueryEliminateDistinct);

	// normalize window operator's project list
	Query *pqueryWindowPlNormalized =
		CQueryMutators::NormalizeWindowProjList(mp, md_accessor, pqueryFixedWindowFrameEdge);
	...
	Query *new_query =
		CQueryMutators::NormalizeGroupByProjList(mp, md_accessor, pqueryHavingNormalized);
...
//---------------------------------------------------------------------------
TargetEntry *
CQueryMutators::MakeTopLevelTargetEntry(TargetEntry *old_target_entry, ULONG attno)
{
	Var *new_var = gpdb::MakeVar(1,
								 (AttrNumber) attno,
	...
	TargetEntry *new_target_entry = gpdb::MakeTargetEntry(
		(Expr *) new_var, (AttrNumber) attno, old_target_entry->resname, old_target_entry->resjunk);
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::ConvertToDerivedTable(const Query *query,
									  ...
									  BOOL should_fix_having_qual)
{
	...
	Query *new_derived_table_query;
	{
		...
		new_derived_table_query =
			(Query *) RunFixCTELevelsUpMutator((Node *) derived_table_query, &context);
	}
...
CQueryMutators::EliminateDistinctClause(const Query *query)
{
	...
	Query *new_query = ConvertToDerivedTable(
		query, true /*should_fix_target_list*/, true /*should_fix_having_qual*/);
	...
	ForEach(lc, target_entries)
	{
		...
		if (!target_entry->resjunk)
		{
			...
			TargetEntry *new_target_entry = gpdb::MakeTargetEntry(
				(Expr *) new_var, (AttrNumber) resno, target_entry->resname, false);
...
CQueryMutators::NeedsProjListWindowNormalization(const Query *query)
{
	...
	ForEach(lc, query->targetList)
	{
		...
		if (!CTranslatorUtils::IsWindowSpec(
				(Node *) target_entry->expr, query->windowClause, query->targetList) &&
			!IsA(target_entry->expr, WindowRef) && !IsA(target_entry->expr, Var))
		{
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeWindowProjList(IMemoryPool *mp,
										CMDAccessor *md_accessor,
										const Query *query)
{
	Query *query_copy = (Query *) gpdb::CopyObject(const_cast<Query *>(query));
	...
	ForEach(lc, target_entries)
	{
		...
		if (CTranslatorUtils::IsWindowSpec(target_entry, query->windowClause))
		{
			...
			if (!target_entry->resjunk ||
				CTranslatorUtils::IsSortingColumn(target_entry, query->sortClause))
			{
				...
				TargetEntry *new_target_entry_copy = gpdb::MakeTargetEntry(
					(Expr *) new_var, ulResNoNew, target_entry->resname, target_entry->resjunk);
		...
		else
		{
			...
			Expr *pexprNew =
				(Expr *) RunWindowProjListMutator((Node *) target_entry->expr, &context);
			TargetEntry *new_target_entry = gpdb::MakeTargetEntry(
				pexprNew, ulResNoNew, target_entry->resname, target_entry->resjunk);
			new_target_entry->ressortgroupref = target_entry->ressortgroupref;
			new_query->targetList = gpdb::LAppend(new_query->targetList, new_target_entry);
		}
	}
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::RunWindowProjListMutator(Node *node, SContextGrpbyPlMutator *context)
{
	if (NULL == node)
	{
		return NULL;
	...
	if (IsA(node, WindowRef))
	{
		...
		const CWStringConst *str =
			CMDAccessorUtils::PstrWindowFuncName(context->m_md_accessor, mdid_func);
		mdid_func->Release();
	...
	if (IsA(node, Var))
	{
		...
		if (NULL == found_target_entry)
		{
			...
			TargetEntry *target_entry =
				gpdb::MakeTargetEntry((Expr *) gpdb::CopyObject(node),
									  (AttrNumber) resno,
		...
		{
			found_target_entry->resjunk =
				false;  // ensure that the derived target list is not resjunked
			new_var = gpdb::MakeVar(1,
									found_target_entry->resno,
									gpdb::ExprType(node),
									gpdb::ExprTypeMod(node),
	...
	return gpdb::MutateExpressionTree(
		node, (MutatorWalkerFn) CQueryMutators::RunWindowProjListMutator, context);
}
...
//---------------------------------------------------------------------------
void
CQueryMutators::ReassignSortClause(Query *top_level_query, Query *derived_table_query)
{
