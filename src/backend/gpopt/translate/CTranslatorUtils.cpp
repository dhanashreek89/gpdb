...
//---------------------------------------------------------------------------
CDXLIndexDescr *
CTranslatorUtils::GetIndexDescr(IMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
...
//---------------------------------------------------------------------------
CDXLTableDescr *
CTranslatorUtils::GetTableDescr(IMemoryPool *mp,
...
)
{
	...
	if (NULL != is_distributed_table && (IMDRelation::EreldistrHash == distribution_policy ||
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsSirvFunc(IMemoryPool *mp, CMDAccessor *md_accessor, OID func_oid)
{
...
//---------------------------------------------------------------------------
CDXLLogicalTVF *
CTranslatorUtils::ConvertToCDXLLogicalTVF(IMemoryPool *mp,
										  CMDAccessor *md_accessor,
										  CIdGenerator *id_generator,
										  const RangeTblEntry *rte)
{
	...
	if (NULL != rte->funccoltypes)
	{
		...
		column_descrs = GetColumnDescriptorsFromRecord(
			mp, id_generator, rte->eref->colnames, rte->funccoltypes, rte->funccoltypmods);
	}
	...
	else if (NULL != out_arg_types)
	{
		...
		column_descrs =
			GetColumnDescriptorsFromRecord(mp, id_generator, rte->eref->colnames, out_arg_types);
		out_arg_types->Release();
	}
	else
	{
		...
		column_descrs = GetColumnDescriptorsFromBase(
			mp, id_generator, mdid_return_type, default_type_modifier, &func_mdname);
	}

	CMDName *pmdfuncname = GPOS_NEW(mp) CMDName(mp, func->Mdname().GetMDName());

	CDXLLogicalTVF *tvf_dxl =
		GPOS_NEW(mp) CDXLLogicalTVF(mp, mdid_func, mdid_return_type, pmdfuncname, column_descrs);
...
//---------------------------------------------------------------------------
IMdIdArray *
CTranslatorUtils::ResolvePolymorphicTypes(IMemoryPool *mp,
										  ...
										  FuncExpr *funcexpr)
{
	...
	const ULONG num_args =
		num_arg_types < num_args_from_query ? num_arg_types : num_args_from_query;
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetColumnDescriptorsFromRecord(IMemoryPool *mp,
												 CIdGenerator *id_generator,
												 List *col_names,
												 List *col_types,
												 List *col_type_modifiers)
{
	...
	ForThree(col_name, col_names, col_type, col_types, col_type_modifier, col_type_modifiers)
	{
		...
		CWStringDynamic *column_name =
			CDXLUtils::CreateDynamicStringFromCharArray(mp, col_name_char_array);
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetColumnDescriptorsFromBase(IMemoryPool *mp,
											   CIdGenerator *id_generator,
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetColumnDescriptorsFromComposite(IMemoryPool *mp,
													CMDAccessor *md_accessor,
													CIdGenerator *id_generator,
													const IMDType *type)
{
...
//---------------------------------------------------------------------------
CMDColumnArray *
CTranslatorUtils::ExpandCompositeType(IMemoryPool *mp,
									  CMDAccessor *md_accessor,
									  const IMDType *type)
{
...
//---------------------------------------------------------------------------
CMDIdGPDB *
CTranslatorUtils::GetSystemColType(IMemoryPool *mp, AttrNumber attno)
{
	GPOS_ASSERT(FirstLowInvalidHeapAttributeNumber < attno && 0 > attno);
...
//---------------------------------------------------------------------------
WindowExclusion
CTranslatorUtils::Windowexclusion(EdxlFrameExclusionStrategy edxlfes)
{
	GPOS_ASSERT(EdxlfesSentinel > edxlfes);
	ULONG rgrgulMapping[][2] = {{EdxlfesNulls, WINDOW_EXCLUSION_NULL},
...
//---------------------------------------------------------------------------
WindowBoundingKind
CTranslatorUtils::Windowboundkind(EdxlFrameBoundary edxlfb)
{
	GPOS_ASSERT(EdxlfbSentinel > edxlfb);

	ULONG rgrgulMapping[][2] = {{EdxlfbUnboundedPreceding, WINDOW_UNBOUND_PRECEDING},
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorUtils::GetGroupingColidArray(IMemoryPool *mp,
...
)
{
	...
	{
		return CreateGroupingSetsForRollup(
			mp, grouping_clause, num_cols, group_col_pos, group_cols);
	}
	...
	ForEach(grouping_set, grouping_clause->groupsets)
	{
		...
		else
		{
			...
			bset = CreateAttnoSetForGroupingSet(
				mp, grouping_set_list, num_cols, group_col_pos, group_cols);
		}
...
)
{
	...
	ForEach(lc, group_elems)
	{
		...
		if (!IsA(elem_node, GroupClause))
		{
			GPOS_RAISE(gpdxl::ExmaDXL,
					   gpdxl::ExmiQuery2DXLUnsupportedFeature,
					   GPOS_WSZ_LIT("Mixing grouping sets with simple group by lists"));
		}
...
//---------------------------------------------------------------------------
Query *
CTranslatorUtils::FixUnknownTypeConstant(Query *old_query, List *output_target_list)
{
	...
	ForEach(target_entry_cell, old_query->targetList)
	{
		...
		if (!old_target_entry->resjunk)
		{
			if (IsA(old_target_entry->expr, Const) &&
				(GPDB_UNKNOWN == gpdb::ExprType((Node *) old_target_entry->expr)))
			{
				if (NULL == new_query)
				{
				...
				new_target_entry->expr =
					(Expr *) gpdb::CoerceToCommonType(NULL, /* pstate */
													  (Node *) old_node,
													  target_type_oid,
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetDXLColumnDescrArray(IMemoryPool *mp,
										 List *target_list,
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorUtils::GetPosInTargetList(IMemoryPool *mp, List *target_list, BOOL keep_res_junked)
{
	GPOS_ASSERT(NULL != target_list);
...
//---------------------------------------------------------------------------
CDXLColDescr *
CTranslatorUtils::GetColumnDescrAt(IMemoryPool *mp,
								   ...
								   ULONG pos)
{
	...
	else
	{
		CWStringDynamic *alias =
			CDXLUtils::CreateDynamicStringFromCharArray(mp, target_entry->resname);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDummyProjectElem(IMemoryPool *mp,
										 ULONG colid_input,
										 ULONG colid_output,
										 CDXLColDescr *dxl_col_descr)
{
	CMDIdGPDB *original_mdid = CMDIdGPDB::CastMdid(dxl_col_descr->MDIdType());
	CMDIdGPDB *copy_mdid = GPOS_NEW(mp) CMDIdGPDB(
		original_mdid->Oid(), original_mdid->VersionMajor(), original_mdid->VersionMinor());
	...
	CDXLColRef *dxl_colref =
		GPOS_NEW(mp) CDXLColRef(mp, mdname, colid_input, copy_mdid, dxl_col_descr->TypeModifier());
	...
	CDXLNode *dxl_project_element = GPOS_NEW(mp) CDXLNode(
		...
		GPOS_NEW(mp) CDXLScalarProjElem(
			mp, colid_output, GPOS_NEW(mp) CMDName(mp, dxl_col_descr->MdName()->GetMDName())),
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorUtils::GetOutputColIdsArray(IMemoryPool *mp,
									   List *target_list,
...
ULONG
CTranslatorUtils::GetColId(
	ULONG query_level, INT varno, INT var_attno, IMDId *mdid, CMappingVarColId *var_colid_mapping)
{
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsWindowSpec(Node *node, List *window_clause_list, List *target_list)
{
	GPOS_ASSERT(NULL != node);
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsWindowSpec(const TargetEntry *target_entry, List *window_clause_list)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDXLProjElemFromInt8Const(IMemoryPool *mp, CMDAccessor *md_accessor, INT val)
{
	GPOS_ASSERT(NULL != mp);
	const IMDTypeInt8 *md_type_int8 = md_accessor->PtMDType<IMDTypeInt8>();
	md_type_int8->MDId()->AddRef();

	CDXLDatumInt8 *datum_dxl =
		GPOS_NEW(mp) CDXLDatumInt8(mp, md_type_int8->MDId(), false /*fConstNull*/, val);
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsSortingColumn(const TargetEntry *target_entry, List *sort_clause_list)
{
...
//---------------------------------------------------------------------------
TargetEntry *
CTranslatorUtils::GetGroupingColumnTargetEntry(Node *node, List *group_clause, List *target_list)
{
	GPOS_ASSERT(NULL != node);
	List *target_list_subset = gpdb::FindMatchingMembersInTargetList(node, target_list);
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsGroupingColumn(const TargetEntry *target_entry, List *group_clause)
{
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsGroupingColumn(const SortClause *sort_group_clause, List *group_clause)
{
	ListCell *group_clause_cell = NULL;
	ForEach(group_clause_cell, group_clause)
	{
		Node *group_clause_node = (Node *) lfirst(group_clause_cell);
		GPOS_ASSERT(IsA(group_clause_node, GroupClause) &&
					"We currently do not support grouping sets.");
...
//---------------------------------------------------------------------------
List *
CTranslatorUtils::ConvertColidToAttnos(ULongPtrArray *colids, CDXLTranslateContext *translate_ctxt)
{
	GPOS_ASSERT(NULL != colids);
...
//---------------------------------------------------------------------------
UlongToUlongMap *
CTranslatorUtils::MakeNewToOldColMapping(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDXLProjElemConstNULL(IMemoryPool *mp,
											 CMDAccessor *md_accessor,
											 ...
											 const IMDColumn *md_col)
{
	...
	CDXLNode *dxl_project_element =
		CreateDXLProjElemConstNULL(mp, md_accessor, md_col->MDIdType(), colid, col_name);
...
CDXLNode *
CTranslatorUtils::CreateDXLProjElemConstNULL(
	IMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid, ULONG colid, const WCHAR *col_name)
{
...
CDXLNode *
CTranslatorUtils::CreateDXLProjElemConstNULL(
	IMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid, ULONG colid, CHAR *alias_name)
{
	...
	{
		datum_dxl = CMDTypeGenericGPDB::CreateDXLDatumVal(mp,
														  mdid,
														  default_type_modifier,
	...
	CDXLNode *dxl_const_node =
		GPOS_NEW(mp) CDXLNode(mp, GPOS_NEW(mp) CDXLScalarConstValue(mp, datum_dxl));

	return GPOS_NEW(mp)
		CDXLNode(mp, GPOS_NEW(mp) CDXLScalarProjElem(mp, colid, alias_mdname), dxl_const_node);
}
...
//---------------------------------------------------------------------------
void
CTranslatorUtils::CheckAggregateWindowFn(Node *node)
{
	GPOS_ASSERT(NULL != node);
	...
	if (gpdb::AggregateExists(win_ref->winfnoid) &&
		!gpdb::AggHasPrelimOrInvPrelimFunc(win_ref->winfnoid))
	{
		GPOS_RAISE(
			gpdxl::ExmaDXL,
			gpdxl::ExmiQuery2DXLUnsupportedFeature,
...
//---------------------------------------------------------------------------
void
CTranslatorUtils::UpdateGrpColMapping(IMemoryPool *mp,
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::RelHasTriggers(IMemoryPool *mp,
								 CMDAccessor *md_accessor,
...
CTranslatorUtils::GetAssertErrorMsgs(CDXLNode *assert_constraint_list)
{
	...
	for (ULONG ul = 0; ul < num_constraints; ul++)
	{
		...
		error_msgs_list = gpdb::LAppend(
			error_msgs_list,
			gpdb::MakeStringValue(CreateMultiByteCharStringFromWCString(error_msg->GetBuffer())));
	}
...
BOOL
CTranslatorUtils::ShouldCreateStatsBucket(OID att_type_oid)
{
	if (att_type_oid != TEXTOID && att_type_oid != CHAROID && att_type_oid != VARCHAROID &&
		att_type_oid != BPCHAROID)
		return true;
