	...
	)
{
	...
	{
		lead_edge->AddChild(CreateDXLNodeFromWindowFrameEdgeVal(pwindowframe->lead->val, var_colid_mapping, new_scalar_proj_list, has_distributed_tables));
	}

	if (NULL != pwindowframe->trail->val)
	{
		trail_edge->AddChild(CreateDXLNodeFromWindowFrameEdgeVal(pwindowframe->trail->val, var_colid_mapping, new_scalar_proj_list, has_distributed_tables));
	}
