...
static const OID lead_func_oids[] = {
	7011, 7074, 7075, 7310, 7312, 7314, 7316, 7318, 7320, 7322, 7324, 7326, 7328, 7330, 7332,
	7334, 7336, 7338, 7340, 7342, 7344, 7346, 7348, 7350, 7352, 7354, 7356, 7358, 7360, 7362,
	7364, 7366, 7368, 7370, 7372, 7374, 7376, 7378, 7380, 7382, 7384, 7386, 7388, 7390, 7392,
	7394, 7396, 7398, 7400, 7402, 7404, 7406, 7408, 7410, 7412, 7414, 7416, 7418, 7420, 7422,
	7424, 7426, 7428, 7430, 7432, 7434, 7436, 7438, 7440, 7442, 7444, 7446, 7448, 7450, 7452,
	7454, 7456, 7458, 7460, 7462, 7464, 7466, 7468, 7470, 7472, 7474, 7476, 7478, 7480, 7482,
...
static const OID lag_func_oids[] = {
	7675, 7491, 7493, 7495, 7497, 7499, 7501, 7503, 7505, 7507, 7509, 7511, 7513, 7515, 7517,
	7519, 7521, 7523, 7525, 7527, 7529, 7531, 7533, 7535, 7537, 7539, 7541, 7543, 7545, 7547,
	7549, 7551, 7553, 7555, 7557, 7559, 7561, 7563, 7565, 7567, 7569, 7571, 7573, 7575, 7577,
	7579, 7581, 7583, 7585, 7587, 7589, 7591, 7593, 7595, 7597, 7599, 7601, 7603, 7605, 7607,
	7609, 7611, 7613, 7615, 7617, 7619, 7621, 7623, 7625, 7627, 7629, 7631, 7633, 7635, 7637,
	7639, 7641, 7643, 7645, 7647, 7649, 7651, 7653, 7655, 7657, 7659, 7661, 7663, 7665, 7667,
...
//
//---------------------------------------------------------------------------
CTranslatorQueryToDXL::CTranslatorQueryToDXL(IMemoryPool *mp,
	  ...
	  m_cteid_at_current_query_level_map(NULL)
{
	...
	if (NULL != query_level_to_cte_map)
	{
		...
		while (cte_list_hashmap_iter.Advance())
		{
			...
			if (cte_query_level < query_level && NULL != cte_list_entry)
			{
				...
				BOOL is_res =
#endif
					m_query_level_to_cte_map->Insert(GPOS_NEW(mp) ULONG(cte_query_level),
													 cte_list_entry);
				GPOS_ASSERT(is_res);
			}
		}
	...
	m_scalar_translator = GPOS_NEW(m_mp) CTranslatorScalarToDXL(m_mp,
																m_md_accessor,
																m_colid_counter,
																m_cte_id_counter,
																m_query_level,
...
CTranslatorQueryToDXL::QueryToDXLInstance(IMemoryPool *mp,
										  CMDAccessor *md_accessor,
										  CIdGenerator *m_colid_counter,
										  CIdGenerator *cte_id_counter,
										  CMappingVarColId *var_colid_mapping,
										  ...
										  HMUlCTEListEntry *query_level_to_cte_map)
{
	return GPOS_NEW(mp) CTranslatorQueryToDXL(mp,
											  md_accessor,
											  m_colid_counter,
											  cte_id_counter,
											  var_colid_mapping,
...
CTranslatorQueryToDXL::CheckSirvFuncsWithoutFromClause(Query *query)
{
	...
	{
		GPOS_RAISE(
			gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("SIRV functions"));
	}
}
...
CTranslatorQueryToDXL::HasSirvFunctions(Node *node) const
{
	...
	List *function_list =
		gpdb::ExtractNodesExpression(node, T_FuncExpr, true /*descendIntoSubqueries*/);
	ListCell *lc = NULL;
	...
	if (CMD_SELECT == query->commandType)
	{
		if (!optimizer_enable_ctas && NULL != query->intoClause)
		{
			GPOS_RAISE(
				gpdxl::ExmaDXL,
				gpdxl::ExmiQuery2DXLUnsupportedFeature,
				GPOS_WSZ_LIT("CTAS. Set optimizer_enable_ctas to on to enable CTAS with GPORCA"));
		}
...
CTranslatorQueryToDXL::TranslateSelectQueryToDXL()
{
	...
	if (NULL != m_query->setOperations)
	{
		...
		child_dxlnode =
			TranslateSetOpToDXL(m_query->setOperations, target_list, output_attno_to_colid_mapping);
	...
	else if (0 != gpdb::ListLength(m_query->windowClause))  // translate window clauses
	{
		...
		child_dxlnode = TranslateWindowToDXL(dxlnode,
											 m_query->targetList,
	...
	{
		child_dxlnode = TranslateGroupingSets(m_query->jointree,
											  m_query->targetList,
											  m_query->groupClause,
											  m_query->hasAggs,
											  sort_group_attno_to_colid_mapping,
	...
	CDXLNode *limit_dxlnode = TranslateLimitToDXLGroupBy(m_query->sortClause,
														 m_query->limitCount,
														 m_query->limitOffset,
														 child_dxlnode,
														 sort_group_attno_to_colid_mapping);
	...
	else
	{
		m_dxl_query_output_cols =
			CreateDXLOutputCols(m_query->targetList, output_attno_to_colid_mapping);
	}
	...
	List *group_clause)
{
	...
	return TranslateTargetListToDXLProject(target_list,
										   join_tree_dxlnode,
										   sort_group_attno_to_colid_mapping,
										   output_attno_to_colid_mapping,
										   group_clause);
...
CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
{
	...
	const RangeTblEntry *rte =
		(RangeTblEntry *) gpdb::ListNth(m_query->rtable, m_query->resultRelation - 1);
	...
	if (!optimizer_enable_dml_triggers &&
		CTranslatorUtils::RelHasTriggers(m_mp, m_md_accessor, md_rel, Edxldmlinsert))
	{
		GPOS_RAISE(gpdxl::ExmaDXL,
				   gpdxl::ExmiQuery2DXLUnsupportedFeature,
	...
	for (ULONG ul = 0; ul < num_table_columns; ul++)
	{
		...
		if (target_list_pos < target_list_length)
		{
			...
			TargetEntry *target_entry =
				(TargetEntry *) gpdb::ListNth(m_query->targetList, target_list_pos);
		...
		CDXLNode *project_elem_dxlnode = CTranslatorUtils::CreateDXLProjElemConstNULL(
...
CTranslatorQueryToDXL::TranslateCTASToDXL()
{
	...
	for (ULONG ul = 0; ul < num_columns; ul++)
	{
		...
		{
			md_colname =
				GPOS_NEW(m_mp) CMDName(m_mp, dxl_ident->MakeDXLColRef()->MdName()->GetMDName());
		}

		GPOS_ASSERT(NULL != md_colname);
		IMDId *mdid = dxl_ident->MDIdType();
		mdid->AddRef();
		CDXLColDescr *dxl_col_descr = GPOS_NEW(m_mp) CDXLColDescr(m_mp,
																  md_colname,
	...
	{
		md_tablespace_name =
			CDXLUtils::CreateMDNameFromCharArray(m_mp, into_clause->tableSpaceName);
	}
	...
	CDXLLogicalCTAS *ctas_dxlop = GPOS_NEW(m_mp) CDXLLogicalCTAS(
		...
		dxl_col_descr_array,
		GPOS_NEW(m_mp)
			CDXLCtasStorageOptions(md_tablespace_name, ctas_commit_action, ctas_storage_options),
		rel_distr_policy,
		distribution_colids,
...
)
{
	...
	ForEach(lc, options)
	{
		...
		CWStringDynamic *name_str =
			CDXLUtils::CreateDynamicStringFromCharArray(m_mp, def_elem->defname);
		...
		CDXLCtasStorageOptions::CDXLCtasOption *dxl_ctas_storage_option = GPOS_NEW(m_mp)
			CDXLCtasStorageOptions::CDXLCtasOption(arg_type, name_str, value_str, is_null_arg);
...
CTranslatorQueryToDXL::GetCtidAndSegmentId(ULONG *ctid, ULONG *segment_id)
{
	...
	*segment_id = CTranslatorUtils::GetColId(m_query_level,
											 m_query->resultRelation,
...
CTranslatorQueryToDXL::GetTupleOidColId()
{
	...
	ULONG tuple_oid_colid = CTranslatorUtils::GetColId(
		m_query_level, m_query->resultRelation, ObjectIdAttributeNumber, mdid, m_var_to_colid_map);
	mdid->Release();
...
CTranslatorQueryToDXL::TranslateDeleteQueryToDXL()
{
	...
	const IMDRelation *md_rel = m_md_accessor->RetrieveRel(table_descr->MDId());
	if (!optimizer_enable_dml_triggers &&
		CTranslatorUtils::RelHasTriggers(m_mp, m_md_accessor, md_rel, Edxldmldelete))
	{
		GPOS_RAISE(gpdxl::ExmaDXL,
				   gpdxl::ExmiQuery2DXLUnsupportedFeature,
	...
	for (ULONG ul = 0; ul < num_of_non_sys_cols; ul++)
	{
		...
		ULONG colid = CTranslatorUtils::GetColId(m_query_level,
												 m_query->resultRelation,
												 mdcol->AttrNum(),
												 mdcol->MDIdType(),
												 m_var_to_colid_map);
	...
	CDXLLogicalDelete *delete_dxlop = GPOS_NEW(m_mp)
		CDXLLogicalDelete(m_mp, table_descr, ctid_colid, segid_colid, delete_colid_array);
...
CTranslatorQueryToDXL::TranslateUpdateQueryToDXL()
{
	...
	const IMDRelation *md_rel = m_md_accessor->RetrieveRel(table_descr->MDId());
	if (!optimizer_enable_dml_triggers &&
		CTranslatorUtils::RelHasTriggers(m_mp, m_md_accessor, md_rel, Edxldmlupdate))
	{
		GPOS_RAISE(gpdxl::ExmaDXL,
				   gpdxl::ExmiQuery2DXLUnsupportedFeature,
	...
	for (ULONG ul = 0; ul < num_of_non_sys_cols; ul++)
	{
		...
		ULONG colid = CTranslatorUtils::GetColId(
			m_query_level, m_query->resultRelation, attno, mdcol->MDIdType(), m_var_to_colid_map);
	...
	CDXLLogicalUpdate *pdxlopupdate = GPOS_NEW(m_mp) CDXLLogicalUpdate(m_mp,
																	   table_descr,
																	   ctid_colid,
																	   segmentid_colid,
																	   delete_colid_array,
...
//---------------------------------------------------------------------------
CDXLWindowFrame *
CTranslatorQueryToDXL::CreateWindowFramForLeadLag(BOOL is_lead_func, CDXLNode *dxl_offset) const
{
	...
	if (NULL == dxl_offset)
	{
		dxl_lead_edge = GPOS_NEW(m_mp) CDXLNode(
			m_mp,
			GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, true /* fLeading */, dxl_frame_lead));
		dxl_trail_edge = GPOS_NEW(m_mp) CDXLNode(
			m_mp,
			GPOS_NEW(m_mp) CDXLScalarWindowFrameEdge(m_mp, false /* fLeading */, dxl_frame_trail));

		dxl_lead_edge->AddChild(
			CTranslatorUtils::CreateDXLProjElemFromInt8Const(m_mp, m_md_accessor, 1 /*iVal*/));
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (is_lead_func || is_lag)
		{
			...
			CDXLWindowSpec *pdxlwsNew =
				GPOS_NEW(m_mp) CDXLWindowSpec(m_mp,
...
//---------------------------------------------------------------------------
CDXLWindowSpecArray *
CTranslatorQueryToDXL::TranslateWindowSpecToDXL(List *window_clause,
												...
												CDXLNode *project_list_dxlnode_node)
{
	...
	ForEach(lc, window_clause)
	{
		...
		ULongPtrArray *part_columns =
			TranslatePartColumns(pwindowspec->partition, sort_col_attno_to_colid_mapping);
		...
		if (NULL != pwindowspec->name)
		{
			CWStringDynamic *alias_str =
				CDXLUtils::CreateDynamicStringFromCharArray(m_mp, pwindowspec->name);
		...
		if (0 < gpdb::ListLength(pwindowspec->order))
		{
			...
			sort_col_list_dxl =
				GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarSortColList(m_mp));

			CDXLNodeArray *dxl_sort_cols =
				TranslateSortColumsToDXL(pwindowspec->order, sort_col_attno_to_colid_mapping);
		...
		{
			window_frame = m_scalar_translator->GetWindowFrame((Expr *) pwindowspec->frame,
															   m_var_to_colid_map,
															   project_list_dxlnode_node,
															   &m_has_distributed_tables);
		}

		CDXLWindowSpec *window_spec_dxlnode = GPOS_NEW(m_mp)
			CDXLWindowSpec(m_mp, part_columns, mdname, sort_col_list_dxl, window_frame);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateWindowToDXL(CDXLNode *child_dxlnode,
											List *target_list,
											List *window_clause,
											List *sort_clause,
											IntToUlongMap *sort_col_attno_to_colid_mapping,
											IntToUlongMap *output_attno_to_colid_mapping)
{
	...
	ForEach(lc, target_list)
	{
		...
		CDXLNode *project_elem_dxlnode =
			TranslateExprToDXLProject(target_entry->expr, target_entry->resname);
		ULONG colid = CDXLScalarProjElem::Cast(project_elem_dxlnode->GetOperator())->Id();
		...
		if (!target_entry->resjunk)
		{
			...
			else if (CTranslatorUtils::IsWindowSpec(target_entry, window_clause))
			{
				...
				CDXLNode *new_project_elem_dxlnode = GPOS_NEW(m_mp)
					CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjElem(m_mp, colid, mdname_alias));
	...
	ForBoth(lc, omitted_target_entries, lc_resno, resno_list)
	{
		...
		TargetEntry *te_window_spec = CTranslatorUtils::GetWindowSpecTargetEntry(
			(Node *) target_entry->expr, window_clause, target_list);
		if (NULL != te_window_spec)
		{
			const ULONG colid = CTranslatorUtils::GetColId((INT) te_window_spec->ressortgroupref,
	...
	CDXLWindowSpecArray *window_spec_array = TranslateWindowSpecToDXL(
		window_clause, sort_col_attno_to_colid_mapping, new_child_project_list_dxlnode);
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorQueryToDXL::TranslatePartColumns(List *partition_by_clause,
											IntToUlongMap *col_attno_colid_mapping) const
{
	...
	ForEach(lc, partition_by_clause)
	{
		...
		ULONG colid = CTranslatorUtils::GetColId((INT) sort_group_clause->tleSortGroupRef,
												 col_attno_colid_mapping);
...
//---------------------------------------------------------------------------
CDXLNodeArray *
CTranslatorQueryToDXL::TranslateSortColumsToDXL(List *sort_clause,
												IntToUlongMap *col_attno_colid_mapping) const
{
	...
	ForEach(lc, sort_clause)
	{
		...
		const ULONG colid = CTranslatorUtils::GetColId((INT) sort_group_clause->tleSortGroupRef,
													   col_attno_colid_mapping);
		...
		CDXLScalarSortCol *sc_sort_col_dxlop =
			GPOS_NEW(m_mp) CDXLScalarSortCol(m_mp,
											 colid,
											 op_mdid,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateLimitToDXLGroupBy(List *sort_clause,
												  Node *limit_count,
												  ...
												  IntToUlongMap *grpcols_to_colid_mapping)
{
	...
	BOOL is_limit_top_level =
		(m_is_top_query_dml && 1 == m_query_level) || (m_is_ctas_query && 0 == m_query_level);
	...
	// create limit count
	CDXLNode *limit_count_dxlnode =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarLimitCount(m_mp));
	...
	CDXLNode *limit_offset_dxlnode =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarLimitOffset(m_mp));
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::AddSortingGroupingColumn(TargetEntry *target_entry,
										   ...
										   IntToUlongMap *output_attno_to_colid_mapping)
{
	if (NULL == grpby_cols_bitset)
	{
		...
		if (!has_grouping_sets)
		{
			...
			while (mi.Advance())
			{
				...
				BOOL result =
#endif  // GPOS_DEBUG
					output_attno_to_colid_mapping->Insert(GPOS_NEW(m_mp) INT(*(mi.Key())),
	...
	ForEach(lc, target_list)
	{
		...
		TargetEntry *te_equivalent = CTranslatorUtils::GetGroupingColumnTargetEntry(
			(Node *) target_entry->expr, group_clause, target_list);

		BOOL is_grouping_col =
			grpby_cols_bitset->Get(target_entry->ressortgroupref) ||
			(NULL != te_equivalent && grpby_cols_bitset->Get(te_equivalent->ressortgroupref));
		ULONG colid = 0;
	...
	return GPOS_NEW(m_mp) CDXLNode(m_mp,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateGroupingSets(FromExpr *from_expr,
											 List *target_list,
											 List *group_clause,
											 BOOL has_aggs,
											 IntToUlongMap *sort_grpref_to_colid_mapping,
											 IntToUlongMap *output_attno_to_colid_mapping)
{
	...
	if (NULL == group_clause)
	{
		...
		CDXLNode *select_project_join_dxlnode =
			TranslateSelectProjectJoinToDXL(target_list,
											from_expr,
		...
		CDXLNode *result_dxlnode = CreateSimpleGroupBy(target_list,
													   group_clause,
													   bitset,
													   has_aggs,
	...
	CBitSetArray *bitset_array = CTranslatorUtils::GetColumnAttnosForGroupBy(
		m_mp, group_clause, num_of_cols, grpcol_index_to_colid_mapping, unique_grp_cols_bitset);
	...
	if (1 == num_of_grouping_sets)
	{
		...
		CDXLNode *groupby_dxlnode = CreateSimpleGroupBy(target_list,
														group_clause,
														bitset,
														has_aggs,
														false,  // has_grouping_sets
		...
		CDXLNode *result_dxlnode = CreateDXLProjectGroupingFuncs(target_list,
																 groupby_dxlnode,
																 bitset,
																 output_attno_to_colid_mapping,
	...
	CDXLNode *result_dxlnode = CreateDXLUnionAllForGroupingSets(from_expr,
																target_list,
																group_clause,
																has_aggs,
																bitset_array,
...
)
{
	...
	CDXLNode *select_project_join_dxlnode =
		TranslateSelectProjectJoinForGrpSetsToDXL(target_list,
												  from_expr,
												  sort_groupref_to_colid_producer_mapping,
												  spj_output_attno_to_colid_mapping,
												  group_clause);
	...
	CDXLNode *cte_producer_dxlnode =
		GPOS_NEW(m_mp) CDXLNode(m_mp, cte_prod_dxlop, select_project_join_dxlnode);
	...
	for (ULONG ul = 0; ul < num_of_grouping_sets; ul++)
	{
		...
		IntToUlongMap *spj_consumer_output_attno_to_colid_mapping =
			RemapColIds(m_mp,
						spj_output_attno_to_colid_mapping,
						op_colid_array_cte_producer,
						colid_array_cte_consumer);
		...
		CDXLNode *cte_consumer_dxlnode = GPOS_NEW(m_mp) CDXLNode(
			m_mp, GPOS_NEW(m_mp) CDXLLogicalCTEConsumer(m_mp, cte_id, colid_array_cte_consumer));
		...
		CDXLNode *project_dxlnode =
			CreateDXLProjectNullsForGroupingSets(target_list,
												 groupby_dxlnode,
												 grouping_set_bitset,
												 phmiulSortgrouprefColIdConsumer,
												 groupby_attno_to_colid_mapping,
		...
		ULongPtrArray *colids_outer_array = CTranslatorUtils::GetOutputColIdsArray(
		...
		if (NULL != unionall_dxlnode)
		{
			...
			CDXLColDescrArray *dxl_col_descr_array = CTranslatorUtils::GetDXLColumnDescrArray(
				m_mp, target_list, colids_outer_array, true /* keep_res_junked */);
			...
			CDXLLogicalSetOp *dxl_setop = GPOS_NEW(m_mp)
				CDXLLogicalSetOp(m_mp, EdxlsetopUnionAll, dxl_col_descr_array, input_colids, false);
			unionall_dxlnode =
				GPOS_NEW(m_mp) CDXLNode(m_mp, dxl_setop, project_dxlnode, unionall_dxlnode);
		}
		else
		{
		...
		if (ul == num_of_grouping_sets - 1)
		{
			...
			ForEach(lc, target_list)
			{
				...
				if (0 < sortgroupref &&
					NULL != phmiulSortgrouprefColIdConsumer->Find(&sortgroupref))
				{
					// add the mapping information for sorting columns
					AddSortingGroupingColumn(
						target_entry, sort_grpref_to_colid_mapping, *(*colid_array_inner)[te_pos]);
				}
	...
	return GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalCTEAnchor(m_mp, cte_id), unionall_dxlnode);
}
...
CTranslatorQueryToDXL::DXLDummyConstTableGet() const
{
	...
	CDXLLogicalConstTable *dxlop = GPOS_NEW(m_mp)
		CDXLLogicalConstTable(m_mp, dxl_col_descr_array, dispatch_identifier_datum_arrays);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateSetOpToDXL(Node *setop_node,
										   List *target_list,
										   IntToUlongMap *output_attno_to_colid_mapping)
{
	...
	ULongPtrArray *output_colids = CTranslatorUtils::GenerateColIds(
		m_mp, target_list, mdid_array_leftchild, leftchild_array, outer_ref_array, m_colid_counter);
	GPOS_ASSERT(output_colids->Size() == leftchild_array->Size());

	GPOS_DELETE_ARRAY(colid);
	GPOS_DELETE_ARRAY(outer_ref_array);

	BOOL is_cast_across_input = SetOpNeedsCast(target_list, mdid_array_leftchild) ||
	...
	CDXLNode *dxlnode = CreateDXLSetOpFromColumns(setop_type,
												  target_list,
												  output_colids,
												  input_colids,
												  children_dxlnodes,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::CreateDXLSetOpFromColumns(EdxlSetOpType setop_type,
												 ...
												 BOOL keep_res_junked) const
{
	...
	ULongPtrArray *output_col_pos =
		CTranslatorUtils::GetPosInTargetList(m_mp, output_target_list, keep_res_junked);
	...
	for (ULONG ul = 0; ul < num_of_cols; ul++)
	{
		...
		if (!is_col_exists)
		{
			...
			output_col_descr =
				CTranslatorUtils::GetColumnDescrAt(m_mp, target_entry, colid_output, ul + 1);
		}
		else
		{
			...
			output_col_descr =
				CTranslatorUtils::GetColumnDescrAt(m_mp, target_entry, colid_union_output, ul + 1);
			CDXLNode *project_elem_dxlnode = CTranslatorUtils::CreateDummyProjectElem(
	...
	CDXLLogicalSetOp *dxlop = GPOS_NEW(m_mp)
		CDXLLogicalSetOp(m_mp, setop_type, output_col_descrs, input_colids, is_cast_across_input);
...
//---------------------------------------------------------------------------
BOOL
CTranslatorQueryToDXL::SetOpNeedsCast(List *target_list, IMdIdArray *input_col_mdids) const
{
	GPOS_ASSERT(NULL != input_col_mdids);
	GPOS_ASSERT(NULL != target_list);
	GPOS_ASSERT(input_col_mdids->Size() <=
										   ...
										   List *target_list)
{
	...
	if (IsA(child_node, RangeTblRef))
	{
		...
		{
			Query *query_derived_tbl =
				CTranslatorUtils::FixUnknownTypeConstant(rte->subquery, target_list);
			GPOS_ASSERT(NULL != query_derived_tbl);

			CMappingVarColId *var_colid_mapping = m_var_to_colid_map->CopyMapColId(m_mp);
			CTranslatorQueryToDXL query_to_dxl_translator(m_mp,
														  m_md_accessor,
														  m_colid_counter,
														  m_cte_id_counter,
														  var_colid_mapping,
			...
			m_has_distributed_tables =
				m_has_distributed_tables || query_to_dxl_translator.HasDistributedTables();
	...
	else if (IsA(child_node, SetOperationStmt))
	{
		IntToUlongMap *output_attno_to_colid_mapping = GPOS_NEW(m_mp) IntToUlongMap(m_mp);
		CDXLNode *dxlnode =
			TranslateSetOpToDXL(child_node, target_list, output_attno_to_colid_mapping);
...
CTranslatorQueryToDXL::TranslateFromExprToDXL(FromExpr *from_expr)
{
	...
	else
	{
		...
		else
		{
			...
			dxlnode =
				GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalJoin(m_mp, EdxljtInner));
	...
	if (1 >= gpdb::ListLength(from_expr->fromlist))
	{
		...
		{
			CDXLNode *select_dxlnode =
				GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalSelect(m_mp));
...
{
	GPOS_ASSERT(!(RTE_RELATION == rtekind || RTE_CTE == rtekind || RTE_FUNCTION == rtekind ||
				  RTE_SUBQUERY == rtekind || RTE_VALUES == rtekind));
	...
	ForEach(lc_tuple, tuples_list)
	{
		...
		ForEach(lc_column, tuple_list)
		{
			...
			if (IsA(expr, Const))
			{
				...
				CWStringDynamic *alias_str =
					CDXLUtils::CreateDynamicStringFromCharArray(m_mp, col_name_char_array);
				CMDName *mdname = GPOS_NEW(m_mp) CMDName(m_mp, alias_str);
				GPOS_DELETE(alias_str);

				CDXLColDescr *dxl_col_descr =
					GPOS_NEW(m_mp) CDXLColDescr(m_mp,
												mdname,
												colid,
												col_pos_idx + 1 /* attno */,
		...
		dxlnodes->Append(TranslateColumnValuesToDXL(
			dxl_datum_array, dxl_column_descriptors, project_elem_dxlnode_array));
		if (fAllConstant)
		{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateColumnValuesToDXL(CDXLDatumArray *dxl_datum_array_const_tbl_get,
												  ...
												  CDXLNodeArray *project_elem_dxlnode_array) const
{
	...
	else
	{
		...
		CDXLLogicalConstTable *dxlop = GPOS_NEW(m_mp)
			CDXLLogicalConstTable(m_mp, dxl_column_descriptors, dxl_datum_arrays_const_tbl_get);
	...
	CDXLNode *project_list_dxlnode = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjList(m_mp), project_elem_dxlnode_array);

	CDXLNode *project_dxlnode = GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLLogicalProject(m_mp), project_list_dxlnode, const_tbl_get_dxlnode);
...
)
{
	...
	if (!IsA(rte->funcexpr, FuncExpr))
	{
		...
		CDXLNode *project_elem_dxlnode = TranslateExprToDXLProject(
			(Expr *) rte->funcexpr, rte->eref->aliasname, true /* insist_new_colids */);
		project_list_dxlnode->AddChild(project_elem_dxlnode);
	...
	CDXLLogicalTVF *tvf_dxlop =
		CTranslatorUtils::ConvertToCDXLLogicalTVF(m_mp, m_md_accessor, m_colid_counter, rte);
	...
	ForEach(lc, func_expr->args)
	{
		...
		CDXLNode *func_expr_arg_dxlnode = m_scalar_translator->CreateScalarOpFromExpr(
			(Expr *) arg_node, m_var_to_colid_map, &m_has_distributed_tables);
										 ...
										 ULONG current_query_level)
{
	...
	m_var_to_colid_map->LoadCTEColumns(current_query_level,
									   rt_index,
									   colid_array_cte_consumer,
												   ...
												   ULONG current_query_level)
{
	...
	m_var_to_colid_map->LoadDerivedTblColumns(current_query_level,
											  rt_index,
											  query_output_cols_dxlnode_array,
...
{
	CDXLNode *scalar_dxlnode = m_scalar_translator->CreateScalarOpFromExpr(
...
CTranslatorQueryToDXL::TranslateJoinExprInFromToDXL(JoinExpr *join_expr)
{
	...
	CDXLNode *join_dxlnode =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalJoin(m_mp, join_type));
	...
	ForBoth(lc_node, rte->joinaliasvars, lc_col_name, alias->colnames)
	{
		...
		CDXLNode *project_elem_dxlnode =
			TranslateExprToDXLProject((Expr *) join_alias_node, col_name_char_array);
		project_list_dxlnode->AddChild(project_elem_dxlnode);
													   ...
													   BOOL is_expand_aggref_expr)
{
	...
	ForEach(lc, target_list)
	{
		...
		{
			vars_list =
				gpdb::ListConcat(vars_list,
								 gpdb::ExtractNodesExpression((Node *) target_entry->expr,
															  T_Var,
	...
	ForEach(lc, omitted_te_list)
	{
		...
		TargetEntry *te_grouping_col = CTranslatorUtils::GetGroupingColumnTargetEntry(
			(Node *) target_entry->expr, plgrpcl, target_list);
		if (NULL != te_grouping_col)
		{
			const ULONG colid = CTranslatorUtils::GetColId((INT) te_grouping_col->ressortgroupref,
														   sort_grpref_to_colid_mapping);
	...
	) const
{
	...
	ForEach(lc, target_list)
	{
		...
		if (IsA(target_entry->expr, GroupingFunc))
		{
			...
			{
				CWStringDynamic *alias_str =
					CDXLUtils::CreateDynamicStringFromCharArray(m_mp, target_entry->resname);
		...
		GPOS_ASSERT_IMP(0 == sort_grouping_col_mapping,
	...
	return GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLLogicalProject(m_mp), project_list_dxlnode, child_dxlnode);
}
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::StoreAttnoColIdMapping(IntToUlongMap *attno_to_colid_mapping,
											  INT attno,
											  ULONG colid) const
{
...
//---------------------------------------------------------------------------
CDXLNodeArray *
CTranslatorQueryToDXL::CreateDXLOutputCols(List *target_list,
										   IntToUlongMap *attno_to_colid_mapping) const
{
	...
	ForEach(lc, target_list)
	{
		...
		CDXLColRef *dxl_colref =
			GPOS_NEW(m_mp) CDXLColRef(m_mp, mdname, colid, mdid_type, type_modifier);
												 ...
												 BOOL insist_new_colids)
{
	...
	CDXLNode *project_elem_dxlnode = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjElem(m_mp, project_elem_id, mdname_alias));
...
CDXLNode *
CTranslatorQueryToDXL::TranslateGroupingFuncToDXL(
	const Expr *expr, CBitSet *bitset, UlongToUlongMap *grpcol_index_to_colid_mapping) const
{
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::ConstructCTEProducerList(List *cte_list, ULONG cte_query_level)
{
	...
	ForEach(lc, cte_list)
	{
		...
		GPOS_ASSERT(NULL != cte_child_dxlnode && NULL != cte_query_output_colds_dxlnode_array &&
		...
		CDXLLogicalCTEProducer *lg_cte_prod_dxlop =
			GPOS_NEW(m_mp) CDXLLogicalCTEProducer(m_mp, m_cte_id_counter->next_id(), colid_array);
		CDXLNode *cte_producer_dxlnode =
		...
		{
			cte_list_entry =
				GPOS_NEW(m_mp) CCTEListEntry(m_mp, cte_query_level, cte, cte_producer_dxlnode);
#ifdef GPOS_DEBUG
			BOOL is_res =
#endif
				m_query_level_to_cte_map->Insert(GPOS_NEW(m_mp) ULONG(cte_query_level),
												 cte_list_entry);
			GPOS_ASSERT(is_res);
		}
		else
										   ...
										   CDXLNode **dxl_cte_anchor_bottom)
{
	...
	for (ULONG ul = num_of_ctes; ul > 0; ul--)
	{
		...
		CDXLNode *cte_anchor_new_dxlnode = GPOS_NEW(m_mp)
			CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalCTEAnchor(m_mp, cte_producer_id));
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorQueryToDXL::GenerateColIds(IMemoryPool *mp, ULONG size) const
{
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorQueryToDXL::ExtractColIds(IMemoryPool *mp, IntToUlongMap *attno_to_colid_mapping) const
{
...
//---------------------------------------------------------------------------
IntToUlongMap *
CTranslatorQueryToDXL::RemapColIds(IMemoryPool *mp,
								   ...
								   ULongPtrArray *to_list_colids) const
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		BOOL result =
#endif  // GPOS_DEBUG
			old_new_col_mapping->Insert(GPOS_NEW(mp) ULONG(*((*from_list_colids)[ul])),
