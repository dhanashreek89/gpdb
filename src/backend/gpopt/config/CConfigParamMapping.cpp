...
CConfigParamMapping::SConfigMappingElem CConfigParamMapping::m_elements[] = {
	...
	{EopttraceForceMultiStageAgg,
	 ...
	 false,  // m_negate_param
	 GPOS_WSZ_LIT("Force optimizer to always pick multistage aggregates when such a plan "
	...
	{EopttraceEnableRedistributeBroadcastHashJoin,
	 ...
	 false,  // m_negate_param
	 GPOS_WSZ_LIT("Enable generating hash join plan where outer child is Redistribute and inner "
				  "child is Broadcast.")},
	...
	{EopttraceApplyLeftOuter2InnerUnionAllLeftAntiSemiJoinDisregardingStats,
	 ...
	 false,  // m_negate_param
	 GPOS_WSZ_LIT("Always apply Left Outer Join to Inner Join UnionAll Left Anti Semi Join without "
				  "looking at stats")},
	...
	{EopttraceForceExpandedMDQAs,
	 ...
	 false,  // m_negate_param
	 GPOS_WSZ_LIT("Always pick plans that expand multiple distinct aggregates into join of single "
				  "distinct aggregate in the optimizer")},
	...
	{EopttraceForceThreeStageScalarDQA,
	 ...
	 false,  // m_negate_param
	 GPOS_WSZ_LIT("Force optimizer to always pick 3 stage aggregate plan for scalar distinct "
				  "qualified aggregate.")},
...
//---------------------------------------------------------------------------
CBitSet *
CConfigParamMapping::PackConfigParamInBitset(IMemoryPool *mp,
...
)
{
	...
	if (!optimizer_enable_outerjoin_to_unionall_rewrite)
	{
		traceflag_bitset->ExchangeSet(
			GPOPT_DISABLE_XFORM_TF(CXform::ExfLeftOuter2InnerUnionAllLeftAntiSemiJoin));
	}
