...
CConstExprEvaluatorProxy::EvaluateExpr(const CDXLNode *dxl_expr)
{
	...
	Expr *result =
		gpdb::EvaluateExpr(expr, gpdb::ExprType((Node *) expr), gpdb::ExprTypeMod((Node *) expr));
	...
	CDXLNode *dxl_result =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarConstValue(m_mp, datum_dxl));
