...
find_minimum_var_level_cbAggref(Aggref *aggref,
                                ...
                                int     sublevelsup);
static bool
find_minimum_var_level_cbPlaceHolderVra(PlaceHolderVar *placeholdervar,
						        void   *context,
                                int     sublevelsup);
static Relids
...
static bool
pull_varnos_cbPlaceHolderVar(PlaceHolderVar *expr, void *context, int sublevelsup)
{
	pull_varnos_context subcontext;
	subcontext.varnos = NULL;

	cdb_walk_vars((Node*)expr->phexpr, pull_varnos_cbVar, NULL, pull_varnos_cbCurrentOf, pull_varnos_cbPlaceHolderVar, &subcontext, sublevelsup);
	if (bms_is_empty(subcontext.varnos) &&
		expr->phlevelsup == sublevelsup)
		((pull_varnos_context *)context)->varnos = bms_add_members(((pull_varnos_context *)context)->varnos, expr->phrels);
	else
		((pull_varnos_context *)context)->varnos = bms_join(((pull_varnos_context *)context)->varnos, subcontext.varnos);
	return false;
}
