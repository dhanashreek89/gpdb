...
namespace gpdxl
{
	...
	class CTranslatorScalarToDXL
	{
		...
		typedef CDXLDatum *(DxlDatumFromDatum)(
	...
	private:
		...
		void TranslateScalarChildren(CDXLNode *dxlnode,
									 List *list,
		...
		CDXLNode *CreateScalarDistCmpFromDistExpr(const Expr *expr,
												  const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar boolean expression node from a GPDB qual list
		CDXLNode *CreateScalarCondFromQual(List *quals,
										   const CMappingVarColId *var_colid_mapping,
										   BOOL *has_distributed_tables);

		// create a DXL scalar comparison node from a GPDB op expression
		CDXLNode *CreateScalarCmpFromOpExpr(const Expr *expr,
											const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar opexpr node from a GPDB expression
		CDXLNode *CreateScalarOpExprFromExpr(const Expr *expr,
											 const CMappingVarColId *var_colid_mapping);

		// translate an array expression
		CDXLNode *TranslateArrayOpExpr(const Expr *expr, const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar array comparison node from a GPDB expression
		CDXLNode *CreateScalarArrayCompFromExpr(const Expr *expr,
												const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar Const node from a GPDB expression
		CDXLNode *CreateScalarConstFromExpr(const Expr *expr,
											const CMappingVarColId *var_colid_mapping);

		// create a DXL node for a scalar nullif from a GPDB Expr
		CDXLNode *CreateScalarNullIfFromExpr(const Expr *expr,
											 const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar boolexpr node from a GPDB expression
		CDXLNode *CreateScalarBoolExprFromExpr(const Expr *expr,
											   const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar boolean test node from a GPDB expression
		CDXLNode *CreateScalarBooleanTestFromExpr(const Expr *expr,
												  const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar nulltest node from a GPDB expression
		CDXLNode *CreateScalarNullTestFromNullTest(const Expr *expr,
												   const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar case statement node from a GPDB expression
		CDXLNode *CreateScalarCaseStmtFromExpr(const Expr *expr,
											   const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar if statement node from a GPDB case expression
		CDXLNode *CreateScalarIfStmtFromCaseExpr(const CaseExpr *case_expr,
												 const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar switch node from a GPDB case expression
		CDXLNode *CreateScalarSwitchFromCaseExpr(const CaseExpr *case_expr,
												 const CMappingVarColId *var_colid_mapping);

		// create a DXL node for a case test from a GPDB Expr.
		CDXLNode *CreateScalarCaseTestFromExpr(const Expr *expr,
											   const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar coalesce node from a GPDB expression
		CDXLNode *CreateScalarCoalesceFromExpr(const Expr *expr,
											   const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar minmax node from a GPDB expression
		CDXLNode *CreateScalarMinMaxFromExpr(const Expr *expr,
											 const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar relabeltype node from a GPDB expression
		CDXLNode *CreateScalarCastFromRelabelType(const Expr *expr,
												  const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar coerce node from a GPDB expression
		CDXLNode *CreateScalarCoerceFromCoerce(const Expr *expr,
											   const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar coerceviaio node from a GPDB expression
		CDXLNode *CreateScalarCoerceFromCoerceViaIO(const Expr *expr,
													const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar array coerce expression node from a GPDB expression
		CDXLNode *CreateScalarArrayCoerceExprFromExpr(const Expr *expr,
													  const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar funcexpr node from a GPDB expression
		CDXLNode *CreateScalarFuncExprFromFuncExpr(const Expr *expr,
												   const CMappingVarColId *var_colid_mapping);
		...
		CDXLNode *PdxlnScWindowref(const Expr *expr, const CMappingVarColId *var_colid_mapping);

		// create a DXL scalar Aggref node from a GPDB expression
		CDXLNode *CreateScalarAggrefFromAggref(const Expr *expr,
											   const CMappingVarColId *var_colid_mapping);

		CDXLNode *CreateScalarIdFromVar(const Expr *expr,
										const CMappingVarColId *var_colid_mapping);

		CDXLNode *CreateInitPlanFromParam(const Param *param) const;

		// create a DXL SubPlan node for a from a GPDB SubPlan
		CDXLNode *CreateSubPlanFromSubPlan(const Expr *expr,
										   const CMappingVarColId *var_colid_mapping);

		CDXLNode *CreatePlanFromParam(const Expr *expr, const CMappingVarColId *var_colid_mapping);

		CDXLNode *CreateDXLNodeFromSublink(const Expr *expr,
										   const CMappingVarColId *var_colid_mapping);

		CDXLNode *CreateScalarSubqueryFromSublink(const SubLink *sublink,
												  const CMappingVarColId *var_colid_mapping);

		CDXLNode *CreateExistSubqueryFromSublink(const SubLink *sublink,
												 const CMappingVarColId *var_colid_mapping);

		CDXLNode *CreateQuantifiedSubqueryFromSublink(const SubLink *sublink,
													  const CMappingVarColId *var_colid_mapping);
		...
		void AddArrayIndexList(CDXLNode *dxlnode,
							   List *list,
		...
		CDXLNode *CreateDXLNodeFromWindowFrameEdgeVal(const Node *node,
													  const CMappingVarColId *var_colid_mapping,
	...
	public:
		// ctor
		CTranslatorScalarToDXL(IMemoryPool *mp,
		...
		CDXLNode *CreateScalarOpFromExpr(const Expr *expr,
										 const CMappingVarColId *var_colid_mapping,
										 BOOL *has_distributed_tables = NULL);
		...
		static CDXLDatum *GetDatumVal(IMemoryPool *mp, CMDAccessor *mda, const Const *constant);

		// translate GPDB datum to CDXLDatum
		static CDXLDatum *GetDatumVal(IMemoryPool *mp,
		...
		static IDatum *GetDatum(IMemoryPool *mp, const IMDType *md_type, BOOL is_null, Datum datum);

		// extract the byte array value of the datum
		static BYTE *GetByteArray(
			IMemoryPool *mp, const IMDType *md_type, BOOL is_null, ULONG len, Datum datum);

		static CDouble GetDoubleValue(IMDId *mdid, BOOL is_null, BYTE *bytes, Datum datum);

		// extract the long int value of a datum
		static LINT GetLintValue(IMDId *mdid, BOOL is_null, BYTE *bytes, ULONG len);
		...
		static CDXLDatum *CreateOidCDXLDatum(
			IMemoryPool *mp, const IMDType *md_type, BOOL is_null, ULONG len, Datum datum);

		// datum to int2 CDXLDatum
		static CDXLDatum *PdxldatumInt2(
			IMemoryPool *mp, const IMDType *md_type, BOOL is_null, ULONG len, Datum datum);

		// datum to int4 CDXLDatum
		static CDXLDatum *PdxldatumInt4(
			IMemoryPool *mp, const IMDType *md_type, BOOL is_null, ULONG len, Datum datum);

		// datum to int8 CDXLDatum
		static CDXLDatum *PdxldatumInt8(
			IMemoryPool *mp, const IMDType *md_type, BOOL is_null, ULONG len, Datum datum);

		// datum to bool CDXLDatum
		static CDXLDatum *CreateBoolCDXLDatum(
			IMemoryPool *mp, const IMDType *md_type, BOOL is_null, ULONG len, Datum datum);

		// datum to generic CDXLDatum
		static CDXLDatum *CreateGenericCDXLDatum(IMemoryPool *mp,
												 const IMDType *md_type,
												 INT type_modifier,
												 BOOL is_null,
												 ULONG len,
