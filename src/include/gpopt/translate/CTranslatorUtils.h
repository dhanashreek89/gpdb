...
namespace gpdxl
{
	...
	private:
		...
		static CBitSet *CreateAttnoSetForGroupingSet(IMemoryPool *mp,
													 List *group_elems,
													 ULONG num_cols,
		...
		static CBitSetArray *CreateGroupingSetsForRollup(IMemoryPool *mp,
		...
		static IMdIdArray *ResolvePolymorphicTypes(IMemoryPool *mp,
												   IMdIdArray *mdid_array,
												   List *arg_types,
												   FuncExpr *func_expr);

		// update grouping col position mappings
		static void UpdateGrpColMapping(IMemoryPool *mp,
										UlongToUlongMap *grouping_col_to_pos_map,
										CBitSet *group_cols,
	...
	public:
		...
		static CMDIdGPDB *GetSystemColType(IMemoryPool *mp, AttrNumber attno);
		...
		static CDXLIndexDescr *GetIndexDescr(IMemoryPool *mp,
											 CMDAccessor *md_accessor,
											 IMDId *mdid);

		// translate a RangeTableEntry into a CDXLTableDescr
		static CDXLTableDescr *GetTableDescr(IMemoryPool *mp,
											 CMDAccessor *md_accessor,
		...
		static CDXLLogicalTVF *ConvertToCDXLLogicalTVF(IMemoryPool *mp,
													   CMDAccessor *md_accessor,
													   CIdGenerator *id_generator,
													   const RangeTblEntry *rte);

		// get column descriptors from a record type
		static CDXLColDescrArray *GetColumnDescriptorsFromRecord(IMemoryPool *mp,
																 CIdGenerator *id_generator,
																 List *col_names,
																 List *col_types,
		...
		static CDXLColDescrArray *GetColumnDescriptorsFromBase(IMemoryPool *mp,
															   CIdGenerator *id_generator,
		...
		static CDXLColDescrArray *GetColumnDescriptorsFromComposite(IMemoryPool *mp,
																	CMDAccessor *md_accessor,
																	CIdGenerator *id_generator,
																	const IMDType *md_type);

		// expand a composite type into an array of IMDColumns
		static CMDColumnArray *ExpandCompositeType(IMemoryPool *mp,
												   CMDAccessor *md_accessor,
												   const IMDType *md_type);
		...
		static CBitSetArray *GetColumnAttnosForGroupBy(IMemoryPool *mp,
													   List *group_clause,
													   ULONG num_cols,
													   UlongToUlongMap *group_col_pos,
													   CBitSet *group_cold);
		...
		static ULongPtrArray *GenerateColIds(IMemoryPool *mp,
											 List *target_list,
		...
		static ULongPtrArray *GetOutputColIdsArray(IMemoryPool *mp,
												   List *target_list,
												   IntToUlongMap *attno_to_colid_map);

		// construct an array of column ids for the given group by set
		static ULongPtrArray *GetGroupingColidArray(IMemoryPool *mp,
		...
		static ULONG GetColId(ULONG query_level,
							  INT varno,
							  INT var_attno,
							  IMDId *mdid,
		...
		static BOOL IsWindowSpec(const TargetEntry *target_entry, List *window_clause_list);
		...
		static CDXLNode *CreateDXLProjElemFromInt8Const(IMemoryPool *mp,
														CMDAccessor *md_accessor,
														INT val);
		...
		static BOOL IsGroupingColumn(const SortClause *sort_group_clause, List *group_clause_list);
		...
		static UlongToUlongMap *MakeNewToOldColMapping(IMemoryPool *mp,
		...
		static BOOL IsSirvFunc(IMemoryPool *mp, CMDAccessor *md_accessor, OID func_oid);
		...
		static CDXLNode *CreateDXLProjElemConstNULL(IMemoryPool *mp,
													CMDAccessor *md_accessor,
													IMDId *mdid,
													ULONG colid,
		...
		static void MarkOuterRefs(ULONG *colid,
								  BOOL *is_outer_ref,
								  ULONG num_columns,
								  CDXLNode *node);

		// map DXL Subplan type to GPDB SubLinkType
		static SubLinkType MapDXLSubplanToSublinkType(EdxlSubPlanType dxl_subplan_type);
		...
		// the given relation
		static BOOL RelHasTriggers(IMemoryPool *mp,
								   CMDAccessor *md_accessor,
		...
		static BOOL ShouldCreateStatsBucket(OID att_type_oid);
	};
}  // namespace gpdxl
