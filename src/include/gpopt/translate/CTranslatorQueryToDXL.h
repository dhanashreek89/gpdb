...
namespace gpdxl
{
	...
	class CTranslatorQueryToDXL
	{
		...
		typedef CDXLNode *(CTranslatorQueryToDXL::*DXLNodeToLogicalFunc)(const RangeTblEntry *rte,
																		 ULONG rti,
	...
	private:
		...
		CDXLNode *TranslateSetOpToDXL(Node *setop_node,
									  List *target_list,
		...
		CDXLNode *TranslateWindowToDXL(CDXLNode *child_dxlnode,
									   List *target_list,
									   List *window_clause,
									   List *sort_clause,
		...
		void UpdateLeadLagWinSpecPos(CDXLNode *project_list_dxlnode,
		...
		CDXLNode *TranslateSetOpChild(Node *child_node,
		...
		CDXLNode *TranslateGroupingSets(FromExpr *from_expr,
										List *target_list,
										List *group_clause,
										BOOL has_aggs,
		...
		CDXLNode *CreateDXLProjectGroupingFuncs(List *target_list,
												CDXLNode *child_dxlnode,
												CBitSet *bitset,
												IntToUlongMap *output_attno_to_colid_mapping,
		...
		CDXLNodeArray *TranslateSortColumsToDXL(List *sort_clause,
												IntToUlongMap *col_attno_colid_mapping) const;

		// translate the list of partition-by column identifiers
		ULongPtrArray *TranslatePartColumns(List *sort_clause,
											IntToUlongMap *col_attno_colid_mapping) const;
		...
		CDXLNode *TranslateTargetListToDXLProject(List *target_list,
												  CDXLNode *child_dxlnode,
		...
		CDXLNode *TranslateRTEToDXLLogicalGet(const RangeTblEntry *rte,
											  ULONG rti,
											  ULONG  //current_query_level
		);
		...
		CDXLNode *TranslateColumnValuesToDXL(CDXLDatumArray *dxl_datum_array,
											 CDXLColDescrArray *dxl_column_descriptors,
											 CDXLNodeArray *dxl_project_elements) const;

		// translate a value scan range table entry
		CDXLNode *TranslateValueScanRTEToDXL(const RangeTblEntry *rte,
											 ULONG rti,
											 ULONG  //current_query_level
		);

		// create a dxl node from a array of datums and project elements
		CDXLNode *TranslateTVFToDXL(const RangeTblEntry *rte,
									ULONG rti,
									ULONG  //current_query_level
		);

		// translate a derived table into a DXL logical operator
		CDXLNode *TranslateDerivedTablesToDXL(const RangeTblEntry *rte,
											  ULONG rti,
											  ULONG current_query_level);
		...
		CDXLNodeArray *CreateDXLOutputCols(List *target_list,
		...
		CDXLNode *TranslateSelectProjectJoinToDXL(List *target_list,
												  FromExpr *from_expr,
		...
		CDXLNode *TranslateGroupingFuncToDXL(const Expr *expr,
											 CBitSet *bitset,
											 UlongToUlongMap *grpcol_index_to_colid_mapping) const;
		...
		IntToUlongMap *RemapColIds(IMemoryPool *mp,
	...
	public:
		...
		// factory function
		static CTranslatorQueryToDXL *QueryToDXLInstance(
			IMemoryPool *mp,
			CMDAccessor *md_accessor,
			CIdGenerator *m_colid_counter,
			CIdGenerator *cte_id_counter,
			...
			HMUlCTEListEntry *query_level_to_cte_map =
				NULL  // hash map between query level -> list of CTEs defined at that level
		);
	};
}  // namespace gpdxl
