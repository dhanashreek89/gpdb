...
namespace gpdxl
{
	...
	private:
		...
		void MapLocationsFile(OID oidRel, char **rgszSegFileMap, CdbComponentDatabases *pcdbCompDB);

		// segment mapping for tables with LOCATION gpfdist(s):// or custom protocol
		void MapLocationsFdist(OID oidRel,
							   char **rgszSegFileMap,
		...
		void MapLocationsExecuteOneSegment(INT iTargetSegInd,
										   CHAR *szPrefixedCommand,
										   char **rgszSegFileMap,
										   CdbComponentDatabases *pcdbCompDB);

		// segment mapping for tables with LOCATION gphdfs://
		void MapLocationsHdfs(char **rgszSegFileMap,
							  CdbComponentDatabases *pcdbCompDB,
							  CHAR *szFirstUri);
		...
		static BOOL SetIndexVarAttnoWalker(Node *node,
										   SContextIndexVarAttno *ctxt_index_var_attno_walker);

	public:
		// ctor
		CTranslatorDXLToPlStmt(IMemoryPool *mp,
	...
	private:
		...
		WindowFrame *Pwindowframe(const CDXLWindowFrame *pdxlwf,
		...
		List *TranslateDXLProjectListToHashTargetList(const CDXLNode *project_list_dxlnode,
		...
		void TranslateProjListAndFilter(const CDXLNode *project_list_dxlnode,
		...
		Plan *TranslateDXLBitmapBoolOp(const CDXLNode *bitmap_boolop_dxlnode,
									   CDXLTranslateContext *output_context,
									   const IMDRelation *md_rel,
									   const CDXLTableDescr *table_descr,
									   CDXLTranslateContextBaseTable *base_table_context,
		...
		void TranslateIndexConditions(CDXLNode *index_cond_list_dxlnode,
		...
		List *TranslateDXLIndexFilter(CDXLNode *filter_dxlnode,
									  CDXLTranslateContext *output_context,
									  CDXLTranslateContextBaseTable *base_table_context,
									  CDXLTranslationContextArray *ctxt_translation_prev_siblings);

		// translate the assert constraints
		List *TranslateDXLAssertConstraints(CDXLNode *filter_dxlnode,
											CDXLTranslateContext *output_context,
											CDXLTranslationContextArray *child_contexts);

		// translate a CTAS operator
		Plan *TranslateDXLCtas(const CDXLNode *dml_dxlnode,
							   CDXLTranslateContext *output_context,
							   CDXLTranslationContextArray *ctxt_translation_prev_siblings =
