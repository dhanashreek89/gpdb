...
namespace gpdxl
{
	...
	class CQueryMutators
	{
		...
		public:
			...
			// ctor
			SContextHavingQualMutator(IMemoryPool *mp,
		...
		public:
			...
			// ctor
			SContextGrpbyPlMutator(IMemoryPool *mp,
								   CMDAccessor *md_accessor,
								   Query *query,
								   List *groupby_target_list)
				: m_mp(mp),
	...
	public:
		...
		// normalize query
		static Query *NormalizeQuery(IMemoryPool *mp,
									 CMDAccessor *md_accessor,
									 const Query *query,
									 ULONG query_level);
		...
		static Query *NormalizeWindowProjList(IMemoryPool *mp,
											  CMDAccessor *md_accessor,
											  const Query *query);
		...
		static Query *NormalizeGroupByProjList(IMemoryPool *mp,
											   CMDAccessor *md_accessor,
											   const Query *query);
		...
		static Var *MakeVarInDerivedTable(Node *node, SContextHavingQualMutator *context);

		// check if a matching node exists in the list of target entries
		static Node *FindNodeInTargetEntries(Node *node, SContextHavingQualMutator *context);

		// increment the levels up of outer references
		static Var *IncrLevelsUpInVar(Var *var);

		// pull up having clause into a select
		static Query *NormalizeHaving(IMemoryPool *mp,
									  CMDAccessor *md_accessor,
									  const Query *query);
		...
		static Node *RunFixCTELevelsUpMutator(Node *node, SContextIncLevelsupMutator *context);
		...
		static Node *RunGroupByProjListMutator(Node *node, SContextGrpbyPlMutator *context);
		...
		static Node *RunHavingQualMutator(Node *node, SContextHavingQualMutator *context);
