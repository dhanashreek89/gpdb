...
private:
	...
	static void LogExceptionMessageAndDelete(CHAR *err_buf,
											 ULONG severity_level = CException::ExsevInvalid);
	...
	static COptimizerConfig *CreateOptimizerConfig(IMemoryPool *mp, ICostModel *cost_model);
	...
	static PlannedStmt *ConvertToPlanStmtFromDXL(IMemoryPool *mp,
												 CMDAccessor *md_accessor,
												 const CDXLNode *dxlnode,
												 bool can_set_tag);

	// load search strategy from given path
	static CSearchStageArray *LoadSearchStrategy(IMemoryPool *mp, char *path);

	// allocate memory for string
	static CHAR *SzAllocate(IMemoryPool *pmp, ULONG ulSize);
	...
	static BOOL FoundException(gpos::CException &exc, const ULONG *exceptions, ULONG size);

	// check if given exception is an unexpected reason for failing to produce a plan
	static BOOL IsUnexpectedFailure(gpos::CException &exc);

	// check if given exception should error out
	static BOOL ShouldErrorOut(gpos::CException &exc);
	...
	static ICostModel *GetCostModel(IMemoryPool *mp, ULONG num_segments);

	// print warning messages for columns with missing statistics
	static void PrintMissingStatsWarning(IMemoryPool *mp,
										 CMDAccessor *md_accessor,
